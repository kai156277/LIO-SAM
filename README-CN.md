...
**实时激光雷达-惯性里程里程计软件包。我们强烈建议用户仔细阅读本文档，并首先使用所提供的数据集对软件包进行测试。该方法的演示视频可在 [YouTube](https://www.youtube.com/watch?v=A0H8CoORZJU)** 上找到。
...
## 目录
  ...
  - [**准备 LiDAR 数据**](#prepare-lidar-data) (must read)

  - [**准备 IMU 数据**](#prepare-imu-data) (must read)

  - [**示例数据**](#sample-datasets)

  - [**运行软件**](#run-the-package)
...
我们设计的系统可维护两个图(Graphs)，运行速度比实时快 10 倍。
  - ```mapOptimization.cpp``` 中的因子图优化了激光雷达里程里程计因子(LiDAR Odometry Factor)和 GPS 因子(GPS Factor)。该因子图在整个测试过程中始终保持不变。
  - ```imuPreintegration.cpp``` 中的因子图优化了 IMU 和激光雷达里程计，并估算了 IMU 偏差(bias)。该系数图会定期重置，并确保以 IMU 频率进行实时里程测量估算。
## Dependency
